...
namespace ManageCinema.DAO
{
    public class AccountDAO
    {
        ...
        public static int Login(string userName, string passWord)
        {
            ...
            DataTable result = DataProvider1.ExecuteQuery(query, new object[] { userName, pass });
        ...
        public static bool UpdatePasswordForAccount(string userName, string passWord, string newPassWord)
        {
            ...
            int result = DataProvider1.ExecuteNonQuery("EXEC USP_UpdatePasswordForAccount @username , @pass , @newPass", new object[] { userName, oldPass, newPass });
        ...
        {
            DataTable data = DataProvider1.ExecuteQuery("Select * from TaiKhoan where userName = '" + userName + "'");
        ...
        {
            DataProvider1.ExecuteQuery("DELETE dbo.TaiKhoan WHERE idNV = '" + idStaff + "'");
        }

        public static DataTable GetAccountList()
        {
            return DataProvider1.ExecuteQuery("USP_GetAccountList");
        }

        public static bool InsertAccount(string username, int accountType, string staffID)
        {
            int result = DataProvider1.ExecuteNonQuery("EXEC USP_InsertAccount @username , @loaiTK , @idnv ", new object[] { username, accountType, staffID });
            return result > 0;
        }
        ...
        public static bool UpdateAccount(string username, int accountType)
        {
            ...
            int result = DataProvider1.ExecuteNonQuery(command);
            return result > 0;
        }

        public static bool DeleteAccount(string username)
        {
            int result = DataProvider1.ExecuteNonQuery("DELETE dbo.TaiKhoan WHERE UserName = N'" + username + "'");
            return result > 0;
        }

        public static DataTable SearchAccountByStaffName(string name)
        {
            return DataProvider1.ExecuteQuery("EXEC USP_SearchAccount @hoten ", new object[] { name });
        }

        public static bool ResetPassword(string username)
        {
            int result = DataProvider1.ExecuteNonQuery("USP_ResetPasswordtAccount @username", new object[] { username });
            return result > 0;
        }
    }
}
