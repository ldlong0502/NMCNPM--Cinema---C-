...
namespace ManageCinema.DAO
{
    public class StaffDAO
    {
        ...
        {
            Staff staff = null;
            DataTable data = DataProvider1.ExecuteQuery("SELECT * FROM dbo.NhanVien WHERE id = '" + id + "'");
        ...
        public static List<Staff> GetStaff()
        {
            ...
            DataTable data = DataProvider1.ExecuteQuery("SELECT * FROM dbo.NhanVien");
            foreach (DataRow item in data.Rows)
            {
        ...
        {
            return DataProvider1.ExecuteQuery("EXEC USP_GetStaff");
        }

        public static bool InsertStaff(string id, string hoTen, DateTime ngaySinh, string diaChi, string sdt, int cmnd)
        {
            int result = DataProvider1.ExecuteNonQuery("EXEC USP_InsertStaff @idStaff , @hoTen , @ngaySinh , @diaChi , @sdt , @cmnd ", new object[] { id, hoTen, ngaySinh, diaChi, sdt, cmnd });
            return result > 0;
        }
        ...
        public static bool DeleteStaff(string id)
        {
            ...
            int result = DataProvider1.ExecuteNonQuery("DELETE dbo.NhanVien WHERE id = '" + id + "'");
            return result > 0;
        }
        ...
        public static DataTable SearchStaffByName(string name)
        {
            ...
            DataTable data = DataProvider1.ExecuteQuery("EXEC USP_SearchStaff @hoTen", new object[] { name });
